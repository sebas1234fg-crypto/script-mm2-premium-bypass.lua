-- =============================================================================
-- PANEL: SΞBΛS | ΞXPLØITS x Itachi v3.7
-- MODULO COMPLETADO CON LOGO INTEGRADO EN ALERTA Y FUENTE UNIFORME
-- =============================================================================
...
-- Identidad Visual del Panel (Actualizado para Delta)
local PanelLogo = "rbxassetid://99579616596711" 
...
RestoreButton.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(MainPanel, TweenInfo.new(0.8, Enum.EasingStyle.Cubic, Enum.EasingDirection.Out), { Size = UDim2.new(0, 420, 0, 390), Position = UDim2.new(0.25, 0, 0.15, 0) }):Play()
end)
...
local ToggleNeonPulseButton = Instance.new("TextButton")

-- =============================================================================
-- INTRO CINEMÁTICA CON MARCA PERSONALIZADA v3.7 (CORREGIDA)
-- =============================================================================
...
local function triggerAction(btn)
    ...
    elseif btn == ShootMurderButton then
        if LP.Character then
            ...
            if gun then
                ...
                if targetRoot and LP.Character:FindFirstChild("Gun") then
                    workspace.CurrentCamera.CFrame = CFrame.new(workspace.CurrentCamera.CFrame.Position, targetRoot.Position)
                    local shootRemote = gun:FindFirstChild("Shoot") or gun:FindFirstChildOfClass("RemoteEvent")
                    if shootRemote then shootRemote:FireServer(targetRoot.Position) else gun:Activate() end
                end
            end
        end
    ...
    elseif btn == RadarButton then
        ...
        RadarButton.Text = radarEnabled and "Radar 2D Portátil: DESACTIVADO" or "Radar 2D Portátil: ACTIVADO"
    ...
    elseif btn == OverlayButton then
        ...
        if not overlayEnabled then
            for _, p in pairs(Players:GetPlayers()) do
                if p.Character and p.Character:FindFirstChild("Head") then local tag = p.Character.Head:FindFirstChild("DeltaInfoTag"); if tag then tag:Destroy() end end
            end
        end
    ...
    elseif btn == AntiAfkButton then
        ...
        if antiAfkEnabled then
            antiAfkConnection = LP.Idled:Connect(function()
                game:GetService("VirtualUser"):Button2Down(Vector2.new(0,0), workspace.CurrentCamera.CFrame); task.wait(1); game:GetService("VirtualUser"):Button2Up(Vector2.new(0,0), workspace.CurrentCamera.CFrame)
            end)
    ...
    elseif btn == GlitchSpotButton then
        ...
        GlitchSpotButton.Text = glitchSpotEnabled protect and "Modo Escondite Glitch: ON" or "Modo Escondite Glitch: OFF"
        local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        if root then
    ...
    elseif btn == TeleportButton then
        ...
        if targetGunDrop then
            ...
            if gunCFrame then local sPos = root.CFrame; root.CFrame = gunCFrame; task.wait(0.12); if root and LP.Character:FindFirstChildOfClass("Humanoid").Health > 0 then root.CFrame = sPos end return end
        end
    ...
    elseif btn == ToggleNeonPulseButton then
        neonPulseEnabled = not neonPulseEnabled
        ToggleNeonPulseButton.BackgroundColor3 = neonPulseEnabled and Color3.fromRGB(40, 140, 40) or Color3.fromRGB(80, 40, 40)
        ToggleNeonPulseButton.Text = neonPulseEnabled and "Efecto Neón Pulsante: ON" or "Efecto Neón Pulsante: OFF"
    end
end

-- ANIMACIÓN EXTRA: EFECTO PULSO NEÓN EN LOS BORDES
RunService.RenderStepped:Connect(function()
    if neonPulseEnabled and MainPanel.Visible then
        local speed = 4
        local alpha = (math.sin(tick() * speed) + 1) / 2
        NeonBorder.Transparency = math.clamp(alpha, 0.1, 0.7)
    else
        NeonBorder.Transparency = 0.4
    end
end)
...
local function setupFloatingSystem(mainButton)
    ...
    mainButton.MouseButton1Down:Connect(function()
        ...
        if isHolding then
            ...
            if existingFloater and existingFloater.Parent then existingFloater:Destroy(); activeFloaters[mainButton] = nil
            else
                local floatBtn = Instance.new("TextButton", ScreenGui); floatBtn.BackgroundColor3 = mainButton.BackgroundColor3
                floatBtn.Text = mainButton.Text; floatBtn.TextColor3 = Color3.fromRGB(255, 255, 255); floatBtn.TextSize = 11; floatBtn.Active = true; floatBtn.Draggable = true
                Instance.new("UICorner", floatBtn).CornerRadius = UDim.new(0, 6); floatBtn.Size = UDim2.new(0, 200, 0, 32); floatBtn.Position = UDim2.new(0.4, 0, 0.4, 0)
                mainButton:GetPropertyChangedSignal("Text"):Connect(function() if floatBtn and floatBtn.Parent then floatBtn.Text = mainButton.Text; floatBtn.BackgroundColor3 = mainButton.BackgroundColor3 end end)
...
local function styleHackBtn(btn, parent, text, yPos, color)
    btn.Parent = parent; btn.Size = UDim2.new(0, 280, 0, 32); btn.Position = UDim2.new(0, 12, 0, yPos)
    btn.BackgroundColor3 = color; btn.Text = text; btn.TextColor3 = Color3.fromRGB(255, 255, 255); btn.TextSize = 12; btn.ZIndex = 4
...
styleHackBtn(ShootMurderButton, CombatFrame, "💥 Disparar al Asesino Ahora", 490, Color3.fromRGB(200, 20, 40))
...
styleHackBtn(CoinButton, ExploitsFrame, "Auto-Farm Monedas (Ultra-Fast): OFF", 10, Color3.fromRGB(160, 120, 0))
styleHackBtn(NoclipButton, ExploitsFrame, "Noclip (Paredes): OFF", 50, Color3.fromRGB(70, 70, 70))
styleHackBtn(JumpButton, ExploitsFrame, "Salto Infinito: DESACTIVADO", 90, Color3.fromRGB(0, 120, 120))
styleHackBtn(SpeedButton, ExploitsFrame, "Velocidad Estable (+X2): OFF", 130, Color3.fromRGB(40, 140, 100))
styleHackBtn(BrightButton, ExploitsFrame, "Brillo Total (FullBright): OFF", 170, Color3.fromRGB(180, 140, 40))
styleHackBtn(FreecamButton, ExploitsFrame, "Cámara Libre Invisible: OFF", 210, Color3.fromRGB(120, 0, 120))
styleHackBtn(InvisibleButton, ExploitsFrame, "Modo Invisible (Fantasma): OFF", 250, Color3.fromRGB(0, 150, 150))
styleHackBtn(AutoVoteButton, ExploitsFrame, "Voto de Mapa Fantasma: OFF", 290, Color3.fromRGB(60, 40, 40))
styleHackBtn(AntiAfkButton, ExploitsFrame, "Anti-AFK Desconexión: OFF", 330, Color3.fromRGB(50, 70, 50))
styleHackBtn(RevealMurdererChatButton, ExploitsFrame, "Revelar Asesino (Chat): OFF", 370, Color3.fromRGB(30, 80, 80))
styleHackBtn(GlitchSpotButton, ExploitsFrame, "Modo Escondite Glitch: OFF", 410, Color3.fromRGB(120, 90, 30))
styleHackBtn(AnimToggleButton, ExploitsFrame, "Animación de Red: ON", 450, Color3.fromRGB(40, 25, 30))
styleHackBtn(ResetButton, ExploitsFrame, "Suicidio de Emergencia (Reset)", 490, Color3.fromRGB(180, 0, 0))

styleHackBtn(ToggleNeonPulseButton, ConfigFrame, "Efecto Neón Pulsante: OFF", 10, Color3.fromRGB(80, 40, 40))

-- SLIDERS DE PERSONALIZACIÓN DE TEMA
SliderLabel.Parent = ConfigFrame; SliderLabel.Size = UDim2.new(0, 280, 0, 15); SliderLabel.Position = UDim2.new(0, 12, 0, 60); SliderLabel.ZIndex = 4
...
SliderFrame.Parent = ConfigFrame; SliderFrame.Size = UDim2.new(0, 280, 0, 12); SliderFrame.Position = UDim2.new(0, 12, 0, 80); SliderFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50); SliderFrame.ZIndex = 4
...
game:GetService("UserInputService").InputChanged:Connect(function(input) if draggingColor and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then updateSliderColor(input) end end)
game:GetService("UserInputService").InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingColor = false end end)

local TransLabel = Instance.new("TextLabel", ConfigFrame); TransLabel.Size = UDim2.new(0, 280, 0, 15); TransLabel.Position = UDim2.new(0, 12, 0, 110); TransLabel.BackgroundTransparency = 1; TransLabel.Text = "Transparencia del Menú:"; TransLabel.TextColor3 = Color3.fromRGB(220, 220, 220); TransLabel.TextSize = 11; TransLabel.TextXAlignment = Enum.TextXAlignment.Left; TransLabel.ZIndex = 4
local TransFrame = Instance.new("Frame", ConfigFrame); TransFrame.Size = UDim2.new(0, 280, 0, 12); TransFrame.Position = UDim2.new(0, 12, 0, 130); TransFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50); TransFrame.ZIndex = 4; Instance.new("UICorner", TransFrame).CornerRadius = UDim.new(0, 4)
...
game:GetService("UserInputService").InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingTrans = false end end)

local DarkLabel = Instance.new("TextLabel", ConfigFrame); DarkLabel.Size = UDim2.new(0, 280, 0, 15); DarkLabel.Position = UDim2.new(0, 12, 0, 160); DarkLabel.BackgroundTransparency = 1; DarkLabel.Text = "Opacidad de Secciones Internas:"; DarkLabel.TextColor3 = Color3.fromRGB(220, 220, 220); DarkLabel.TextSize = 11; DarkLabel.TextXAlignment = Enum.TextXAlignment.Left; DarkLabel.ZIndex = 4
local DarkFrame = Instance.new("Frame", ConfigFrame); DarkFrame.Size = UDim2.new(0, 280, 0, 12); DarkFrame.Position = UDim2.new(0, 12, 0, 180); DarkFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50); DarkFrame.ZIndex = 4; Instance.new("UICorner", DarkFrame).CornerRadius = UDim.new(0, 4)
local DarkButton = Instance.new("TextButton", DarkFrame); DarkButton.Size = UDim2.new(0, 20, 1, 0); DarkButton.BackgroundColor3 = Color3.fromRGB(255, 255, 255); DarkButton.Text = ""; DarkButton.ZIndex = 5; Instance.new("UICorner", DarkButton).CornerRadius = UDim.new(0, 4)

local draggingDark = false
DarkButton.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingDark = true end end)
game:GetService("UserInputService").InputChanged:Connect(function(input)
    if draggingDark and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
        local relativeX = input.Position.X - DarkFrame.AbsolutePosition.X
        local percentage = math.clamp(relativeX / DarkFrame.AbsoluteSize.X, 0, 1)
        DarkButton.Position = UDim2.new(percentage, -10, 0, 0)
        globalDarkness = percentage
        ContentFrame.BackgroundTransparency = globalDarkness
    end
end)
game:GetService("UserInputService").InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then draggingDark = false end end)

-- =============================================================================
-- PROCESOS OPERATIVOS DEL JUEGO (AUTOMATISMOS Y BUCLE DE EVENTOS)
-- =============================================================================
...
-- BUCLE OPERATIVO DE RENDER (ESP, TRACERS CORREGIDOS AL PECHO, RADAR Y GUN BEACON AGRESIVO)
RunService.RenderStepped:Connect(function()
    local Camera = workspace.CurrentCamera
        ...
        if gunFound then
            ...
            if gunCFrame then
                if not beaconPart or not beaconPart.Parent then
                    ...
                    beaconPart.Name = "GunBeaconPart"
                    beaconPart.Size = Vector3.new(1, 1000, 1)
                    beaconPart.Anchored = true; beaconPart.CanCollide = false; beaconPart.Material = Enum.Material.Neon
                    beaconPart.Color = Color3.fromRGB(0, 180, 255); beaconPart.Transparency = 0.3
                end
                beaconPart.CFrame = CFrame.new(gunCFrame.Position + Vector3.new(0, 500, 0))
            end
        else if beaconPart then beaconPart:Destroy(); beaconPart = nil end end
    else if beaconPart then beaconPart:Destroy(); beaconPart = nil end end
    ...
    if knifeTrajectoryEnabled and localRoot and LP.Character:FindFirstChild("Knife") then
        ...
        if knife:FindFirstChild("Handle") then
            ...
            laser.Name = "DeltaLaser"; laser.Anchored = true; laser.CanCollide = false
            laser.Color = Color3.fromRGB(255, 0, 0); laser.Material = Enum.Material.Neon; laser.Transparency = 0.2
    ...
    else if workspace:FindFirstChild("DeltaLaser") then workspace.DeltaLaser:Destroy() end end
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= LP then
            ...
            if cache and p.Character and p.Character:FindFirstChild("HumanoidRootPart") and p.Character:FindFirstChild("Head") then
                ...
                if p.Backpack:FindFirstChild("Knife") or p.Character:FindFirstChild("Knife") then color = Color3.fromRGB(255, 0, 0); roleText = "ASESINO"
                ...
                else local tag = head:FindFirstChild("DeltaInfoTag"); if tag then tag:Destroy() end end
                ...
                else if cache.Blip then cache.Blip.Visible = false end end
                
                if espEnabled and cache.Highlight then cache.Highlight.Adornee = p.Character; cache.Highlight.FillColor = color; cache.Highlight.OutlineColor = color; cache.Highlight.Enabled = true
                else if cache.Highlight then cache.Highlight.Enabled = false end end
                
                if tracersEnabled and cache.Tracer then
                    local chestPart = p.Character:FindFirstChild("UpperTorso") or p.Character:FindFirstChild("Torso")
                    local targetWorldPos = chestPart and chestPart.Position or (pRoot.Position + Vector3.new(0, 1.5, 0))
                    local vector, onScreen = Camera:WorldToViewportPoint(targetWorldPos)
                    if onScreen then
                        local sPos = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y); local distance = (Vector2.new(vector.X, vector.Y) - sPos).Magnitude
                        cache.Tracer.BackgroundColor3 = color; cache.Tracer.Size = UDim2.new(0, distance, 0, 2); cache.Tracer.Position = UDim2.new(0, (sPos.X + vector.X) / 2 - distance / 2, 0, (sPos.Y + vector.Y) / 2); cache.Tracer.Rotation = math.deg(math.atan2(vector.Y - sPos.Y, vector.X - sPos.X)); cache.Tracer.Visible = true
                    else cache.Tracer.Visible = false end
                else if cache.Tracer then cache.Tracer.Visible = false end end
            else if cache then if cache.Highlight then cache.Highlight.Enabled = false end; if cache.Tracer then cache.Tracer.Visible = false end; if cache.Blip then cache.Blip.Visible = false end end end
        end
    end
end)
...
RunService.Stepped:Connect(function()
    if noclipEnabled and LP.Character then for _, part in pairs(LP.Character:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide = false end end end
end)

game:GetService("UserInputService").JumpRequest:Connect(function()
    if jumpEnabled and LP.Character and LP.Character:FindFirstChildOfClass("Humanoid") then LP.Character:FindFirstChildOfClass("Humanoid"):ChangeState(Enum.HumanoidStateType.Jumping) end
end)

RunService.Heartbeat:Connect(function()
    if LP.Character then
        local humanoid = LP.Character:FindFirstChildOfClass("Humanoid")
        if humanoid then if speedEnabled then humanoid.WalkSpeed = 50 else humanoid.WalkSpeed = 16 end end
    end
end)
...
RunService.Heartbeat:Connect(function()
    if autoHoldEnabled and LP.Character then
        ...
        if hum then
            if knife then hum:EquipTool(knife) elseif gun then hum:EquipTool(gun) end
        end
    end
end)

-- TECLA PARA MOSTRAR/OCULTAR EL MENÚ (INSERT)
game:GetService("UserInputService").InputBegan:Connect(function(input, processed)
    if not processed and input.KeyCode == Enum.KeyCode.Insert then
        if RestoreButton.Visible == false then
...
-- =============================================================================
-- NOTIFICACIÓN ESTILO AMISTAD CON EL LOGO ORIGINAL VINCULADO
-- =============================================================================
local function showNotification()
    ...
    NotificationFrame.Name = "SebasNotification"
    NotificationFrame.Parent = ScreenGui
    NotificationFrame.Position = UDim2.new(1, 20, 0.85, 0)
    NotificationFrame.Size = UDim2.new(0, 270, 0, 60)
    NotificationFrame.BackgroundColor3 = Color3.fromRGB(24, 20, 24)
    NotificationFrame.ZIndex = 200
    
    Corner.CornerRadius = UDim.new(0, 8)
    Corner.Parent = NotificationFrame
    
    Border.Color = currentThemeColor
    Border.Thickness = 2
    Border.Parent = NotificationFrame
    
    -- Icono configurado con la variable global PanelLogo corregida para Delta
    Icon.Parent = NotificationFrame
    Icon.BackgroundTransparency = 1
    Icon.Position = UDim2.new(0, 12, 0.5, -18)
    Icon.Size = UDim2.new(0, 36, 0, 36)
    Icon.Image = PanelLogo
    Icon.ImageColor3 = Color3.fromRGB(255, 255, 255)
    Icon.ZIndex = 201
    
    -- Texto exacto con los colores y rayos solicitados
    MessageText.Parent = NotificationFrame
    MessageText.BackgroundTransparency = 1
    MessageText.Position = UDim2.new(0, 60, 0, 0)
    MessageText.Size = UDim2.new(1, -70, 1, 0)
    MessageText.Font = Enum.Font.GothamBold
    MessageText.Text = "🤝 ¡EXECUTE SUCCESSFUL!\n⚡ <font color='#FF0000'>SΞBΛS | ΞXPLØITS</font> <font color='#000000'>x</font> <font color='#A020F0'>Itachi v3.7</font> ⚡"
    MessageText.RichText = true
    MessageText.TextColor3 = Color3.fromRGB(255, 255, 255)
    MessageText.TextSize = 12
    MessageText.TextXAlignment = Enum.TextXAlignment.Left
    MessageText.ZIndex = 201

    task.spawn(function()
        local tweenIn = TweenService:Create(NotificationFrame, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
...
-- Lanzar la alerta modificada al cargar el panel
showNotification()

print("SΞBΛS | ΞXPLØITS x Itachi v3.7: INYECTADO CORRECTAMENTE.")
